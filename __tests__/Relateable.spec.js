...
describe('Relateable', () => {
  ...
  describe('One-to-many relationships', () => {
    ...
    it('can do self-referential relationships', () => {
      ...
      const users = relations.collect('users')
        .relateToMany('users', { as: 'following', fromMy: ['following'] })
        .relateToMany('users', { as: 'followers', toTheir: ['following'] })
        .fill(_DATA)

      expect(users.find(1).$following).toEqual([_DATA[1], _DATA[2]])
      expect(users.find(1).$followers).toEqual([_DATA[1]])
      expect(users.find(2).$following).toEqual([_DATA[0], _DATA[2]])
      expect(users.find(2).$followers).toEqual([_DATA[0]])
      expect(users.find(3).$following).toEqual([])
      expect(users.find(3).$followers).toEqual([_DATA[0], _DATA[1]])
    })
    it('can deep link', () => {
      const relations = Relateable()
      const authors = relations.collect('authors')
  ...
  describe('Relateablehip validation', () => {
